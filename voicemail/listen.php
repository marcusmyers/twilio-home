...
$gather = $response->gather(
  array(
    "action" => "listen.php?exten=$exten&messages=$msg_list&current_msg=$current_msg",
    "numDigits" => "1",
    "timeout" => "5",
  )
);
...
$message = "Message from $from received on ". date('l jS \of F Y h:i A', strtotime($date));
$gather->say($message);
$gather->play($url);
$gather->redirect("listen.php?exten=$exten&messages=$msg_list&current_msg=$current_msg&Digits=1");
print $response;
?>
